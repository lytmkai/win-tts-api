...
func main() {
	...
	fmt.Println("📌 监听地址: http://localhost:8080/tts")
	fmt.Println("📝 支持 POST，内容类型：application/json 或 application/x-www-form-urlencoded")
	fmt.Println("💡 示例（JSON）:")
	fmt.Println(`   curl -X POST http://localhost:8080/tts -H "Content-Type: application/json" -d '{"text":"你好，世界！"}'`)
	fmt.Println("💡 示例（表单）:")
	fmt.Println(`   curl -X POST http://localhost:8080/tts -d "text=欢迎使用 Go TTS"`)
